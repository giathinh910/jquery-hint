var TBHint = function (args) {
    ...
    this.init = function () {
        // Check if hint group exist
        if ($('[data-tb-hint-group="' + this.args.group + '"]').length == 0) {
            return;
        }
        // Get all sorted els in group
        this.vars.els = this.utils.sortByDataOrder($('[data-tb-hint-group="' + this.args.group + '"]'));
        // Start
        this.startLoopHint();
    };
    this.utils = {
        ...
        message: {
            show: function ($point, nextButtonText) {
                ...
                $point.find('.tb-button.next').text(nextButtonText);
            },
            hide: function ($point) {
